...
interface NumberInputProps {
  ...
  max?: number;
  withIcon?: boolean;
  icon?: 'Bedrooms' | 'Bathrooms' | 'ParkingSpaces' | 'InternalPropertySize' | 'ExternalPropertySize';
}
...
const NumberInput: React.FC<NumberInputProps> = ({
  ...
  return (
      <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'left'}}>
        <label className="text-input-label number">{withIcon? icon : label}</label>
        <div className={`text-input-container number ${className}`} style={{ display: 'flex'}}>
        {withIcon && <span className="number-icon">{icon? getIcon(icon) : 'icon is missing'}</span>}
          <input
            type="number"
            value={value}
            onChange={onChange}
