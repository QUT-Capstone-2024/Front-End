...
import { Card, CardContent, Box, Typography, Modal, Select, MenuItem, Dialog, DialogTitle, DialogContent, DialogContentText, TextField, DialogActions } from "@mui/material";
...
const ImageApproval: React.FC = () => {
  ...
  const fetchPropertyData = async () => {
    ...
    try {
      ...
      if (details) setPropertyCategories(generatePropertyCategories(details));
  ...
  // Functions for approving and rejecting images
  const handleImageAction = async (imageId: number, action: "APPROVE" | "REJECT") => {
    ...
    try {
      await updateImageStatus(selectedPropertyId, imageId, action === "APPROVE" ? "APPROVED" : "REJECTED", action === "REJECT" ? currentRejectionReason : "", token);
      setQueuedImages(prev => prev.filter(img => img.id !== imageId));
      if (action === "REJECT") closeRejectionDialog();
      fetchPropertyData();
    } catch (error) { setError("Failed to update image status."); }
  };

  const handleRejectImage = (image: Image) => { setCurrentRejectionImage(image); setRejectionDialogOpen(true); };
  const confirmRejection = async () => { if (currentRejectionImage && currentRejectionReason && token) handleImageAction(currentRejectionImage.id, "REJECT"); };
  const closeRejectionDialog = () => { setRejectionDialogOpen(false); setCurrentRejectionReason(""); };
  const handleBulkAction = async (action: "APPROVE" | "REJECT") => {
    if (!selectedPropertyId || !token) return setError("User is not authenticated or property is not selected.");
    try {
      await Promise.all(queuedImages.map(img => updateImageStatus(selectedPropertyId, img.id, action === "APPROVE" ? "APPROVED" : "REJECTED", action === "REJECT" ? prompt("Please enter a rejection comment for all images:") || "" : "", token)));
      setQueuedImages([]); fetchPropertyData();
    } catch (error) { setError("Failed to update image statuses."); }
  };

  // Functions for editing image category
  ...
  const handleCategoryClick = (image: Image) => { setImageToEdit(image); setCurrentImageTag(image.imageTag); setIsEditModalOpen(true); };
    ...
    <Box sx={{ padding: "20px", backgroundColor: "#e2eaf1", display: "flex", justifyContent: "center", alignItems: "flex-start", minHeight: "100vh" }}>
      ...
      <Dialog open={rejectionDialogOpen} onClose={closeRejectionDialog}>
        ...
        <DialogContent>
          ...
          <TextField autoFocus margin="dense" id="rejectionReason" label="Rejection Reason" type="text" fullWidth variant="outlined" value={currentRejectionReason} onChange={(e) => setCurrentRejectionReason(e.target.value)} sx={{ mt: 2 }} />
        </DialogContent>
        <DialogActions>
