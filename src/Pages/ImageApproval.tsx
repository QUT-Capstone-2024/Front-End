  ...
  const fetchPropertyData = async () => {
    if (selectedPropertyId && token) {
      try {
        ...
        const queued = fetchedImages.filter((image: Image) => image.imageStatus === 'PENDING');
          ...
          <>
            <CardContent sx={{ paddingBottom: '16px' }}>
              ...
              <Box sx={{ position: 'relative', width: '100%', borderRadius: '8px', overflow: 'hidden', mb: 2 }}>
                <img src={(propertyDetails.imageUrls && propertyDetails.imageUrls.length > 0 ? propertyDetails.imageUrls[0] : houseDemoHeroImage)} alt="Property" style={{ width: '100%', height: '375px', objectFit: 'cover' }} />
                <Box sx={{ position: 'absolute', bottom: '40px', left: '20px', width: '90%', backgroundColor: 'rgba(255, 255, 255, 0.45)', padding: '20px', borderRadius: '12px' }}>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '15px', alignItems: 'center', mt: 1 }}>
                    <IconBar 
                      ...
                      internalPropertySize={propertyDetails.propertySize || 0} externalPropertySize={0} />
                  </Box>
                </Box>
              </Box>
